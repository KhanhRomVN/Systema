...
export function InitiatorDetails({ request, requests }: InitiatorDetailsProps) {
  ...
  if (!request.initiator) {
    return (
      <div className="p-4 text-center text-text-secondary bg-secondary/20 rounded-md border border-divider/50 border-dashed m-4">
    ...
    <div className="flex h-full font-mono text-xs overflow-hidden">
      ...
      <div
        className={cn(
          ...
          selectedFrame ? 'w-1/2 border-r border-divider/50' : 'w-full',
        )}
      >
        <div className="p-2 border-b border-divider/50 bg-secondary/20 flex items-center justify-between text-text-secondary shrink-0 h-9">
            ...
            {frames.map((frame, idx) => {
              ...
              return (
                <div
                  key={idx}
                  className={cn(
                    'flex items-start gap-2 p-2 border-b border-divider/10 transition-colors relative',
                    isClickable
      ...
      {selectedFrame && (
        <div className="flex flex-col w-1/2 h-full overflow-hidden animate-in fade-in slide-in-from-right-4 duration-300">
          <div className="p-2 border-b border-divider/50 bg-secondary/20 flex items-center justify-between shrink-0 h-9">
            ...
            <div className="flex items-center gap-1">
              <button
                ...
                className="p-1 hover:bg-background rounded-md transition-colors text-text-secondary hover:text-text-primary"
                title="Search (Regex supported)"
              >
                <Search className="w-3.5 h-3.5" />
              </button>
              <div className="w-px h-3 bg-divider/50 mx-1" />
              <button
                onClick={handleCopy}
                className="p-1 hover:bg-background rounded-md transition-colors text-text-secondary hover:text-text-primary"
