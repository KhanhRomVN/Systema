...
export function analyzeTls(result: TlsScanResult, host?: string): SecurityIssue[] {
  ...
  if (['SSLv2', 'SSLv3', 'TLSv1', 'TLSv1.1'].includes(proto))
    issues.push({
      id: 'tls-old-version',
      title: `Outdated TLS Version (${proto})`,
      severity: 'high',
      description: `${proto} is deprecated and insecure. Only TLS 1.2 and 1.3 should be used.`,
      evidence: proto,
    });
  ...
  if (proto === 'TLSv1.2')
    issues.push({
      id: 'tls-12-only',
      title: 'TLS 1.3 Not Used',
      severity: 'info',
      description: 'Connection negotiated TLS 1.2. TLS 1.3 offers better performance and security.',
      evidence: proto,
    });
  ...
  if (matchedWeak)
    issues.push({
      id: 'tls-weak-cipher',
      title: `Weak Cipher Suite (${matchedWeak})`,
      severity: 'high',
      description: `Cipher "${cipher}" uses ${matchedWeak} which is cryptographically broken.`,
      evidence: cipher,
    });
  ...
  if (cipher && !cipher.includes('ECDHE') && !cipher.includes('DHE'))
    issues.push({
      ...
      severity: 'medium',
      description: `Cipher "${cipher}" does not use ECDHE/DHE key exchange. Past sessions can be decrypted if private key is compromised.`,
      evidence: cipher,
    });
  ...
  if (cert) {
    ...
    if (result.selfSigned)
      issues.push({
        ...
        severity: 'high',
        description:
          'Certificate is self-signed and not trusted by any CA. Should not be used in production.',
      });
    ...
    if (cert.valid_to) {
      ...
      if (daysLeft < 0)
        issues.push({
          ...
          severity: 'high',
          description: `Certificate expired ${Math.abs(daysLeft)} day(s) ago.`,
          evidence: cert.valid_to,
        });
      else if (daysLeft < 30)
        issues.push({
          id: 'cert-expiring-soon',
          title: `Certificate Expiring Soon (${daysLeft}d)`,
          severity: 'medium',
          description: `Certificate expires in ${daysLeft} day(s). Renew before expiry to avoid service disruption.`,
          evidence: cert.valid_to,
        });
    }
    ...
    if (cert.bits && cert.bits < 2048)
      issues.push({
        id: 'cert-weak-key',
        title: `Weak Certificate Key (${cert.bits} bits)`,
        severity: 'high',
        description: `RSA key length ${cert.bits} bits is below the recommended minimum of 2048 bits.`,
        evidence: `${cert.bits} bits`,
      });
    ...
    if (cert.fingerprint && cert.fingerprint.toLowerCase().startsWith('sha1'))
      issues.push({
        ...
        severity: 'high',
        description: 'SHA-1 is cryptographically broken. Certificate should use SHA-256 or higher.',
        evidence: cert.fingerprint,
      });
    ...
    if (cert.infoAccess && !cert.infoAccess.includes('OCSP'))
      issues.push({
        id: 'cert-no-ocsp',
        title: 'No OCSP Responder in Certificate',
        severity: 'medium',
        description:
          'Certificate does not include an OCSP URL. Revocation status cannot be checked online.',
      });
    ...
    if (
      cn.startsWith('*.') &&
      cert.subjectaltname &&
      (cert.subjectaltname.match(/DNS:/g) || []).length === 1
    )
      issues.push({
        id: 'cert-wildcard-only',
        title: 'Wildcard-Only Certificate',
        severity: 'low',
    ...
    if (cert.infoAccess && !cert.infoAccess.includes('CT Precertificate') && !cert.fingerprint256)
      issues.push({
        id: 'cert-no-ct',
        title: 'No Certificate Transparency Evidence',
        severity: 'info',
        description:
          'Certificate may not be logged in CT logs. Modern browsers require CT for public trust.',
      });
  }
...
export function scanRequest(req: NetworkRequest): SecurityIssue[] {
  ...
  if (hsts && !/max-age\s*=\s*(\d+)/.test(hsts))
    issues.push({
      ...
      severity: 'low',
      description: 'HSTS header present but max-age directive is missing or malformed.',
      evidence: hsts,
    });

  if (hsts && parseInt((hsts.match(/max-age\s*=\s*(\d+)/) || [])[1] || '0') < 31536000)
    issues.push({
      id: 'hsts-short-maxage',
      title: 'HSTS max-age Too Short',
      severity: 'low',
      description: 'HSTS max-age is less than 1 year (31536000s). Recommended ≥ 1 year.',
      evidence: hsts,
    });
  ...
  if (csp && csp.includes("'unsafe-inline'"))
    issues.push({
      ...
      severity: 'medium',
      description:
        "CSP contains 'unsafe-inline' which allows inline scripts/styles, weakening XSS protection.",
      evidence: csp.substring(0, 150),
    });
  ...
  if (csp && csp.includes("'unsafe-eval'"))
    issues.push({
      ...
      severity: 'medium',
      description: "CSP contains 'unsafe-eval' which allows eval(), weakening XSS protection.",
      evidence: csp.substring(0, 150),
    });
  ...
  if (csp && csp.includes('*'))
    issues.push({
      id: 'csp-wildcard',
      title: 'CSP Contains Wildcard',
      severity: 'low',
      description:
        'CSP uses wildcard (*) source which may allow loading resources from any origin.',
      evidence: csp.substring(0, 150),
    });
  ...
  if (rp && ['unsafe-url', 'no-referrer-when-downgrade'].includes(rp.toLowerCase()))
    issues.push({
      ...
      severity: 'low',
      description: `Referrer-Policy "${rp}" may leak full URLs to third parties.`,
      evidence: rp,
    });
  ...
  if (!getHeader(res, 'x-xss-protection'))
    issues.push({
      ...
      severity: 'info',
      description:
        'X-XSS-Protection header absent. Legacy browsers may lack reflected XSS filtering.',
    });
  ...
  if (xxp === '0')
    issues.push({
      ...
      severity: 'low',
      description: 'X-XSS-Protection is explicitly set to 0 (disabled).',
      evidence: xxp,
    });
  ...
  if (rawSetCookie) {
    ...
    if (!cookieLower.includes('secure'))
      issues.push({
        ...
        severity: 'high',
        description: 'Set-Cookie missing Secure flag. Cookie may be transmitted over HTTP.',
        evidence: setCookie.substring(0, 120),
      });
    if (!cookieLower.includes('httponly'))
      issues.push({
        ...
        severity: 'medium',
        description: 'Set-Cookie missing HttpOnly. Cookie accessible via JavaScript (XSS risk).',
        evidence: setCookie.substring(0, 120),
      });
    if (!cookieLower.includes('samesite'))
      issues.push({
        ...
        severity: 'low',
        description: 'Set-Cookie has no SameSite attribute. CSRF attacks may be possible.',
        evidence: setCookie.substring(0, 120),
      });
    ...
    if (/expires\s*=\s*[^;]+\s*2[0-9]{3}/i.test(setCookie))
      issues.push({
        ...
        title: 'Cookie With Long Expiry',
        severity: 'info',
        description:
          'Cookie has a far-future expiry date. Long-lived cookies increase session hijacking risk.',
        evidence: setCookie.substring(0, 120),
      });
  }
  ...
  const sensitivePatterns: [RegExp, string, string][] = [
    [
      ...
      'Response body contains a "password" field in plaintext JSON.',
    ],
    [
      /"(secret|api_key|apikey|private_key|client_secret)"\s*:\s*"[^"]{4,}"/i,
      'Secret/Key in Response Body',
      'Response body exposes a secret or API key in JSON.',
    ],
    [
      /\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z]{2,}\b/i,
      'Email Address in Response',
      'Response body contains email addresses which may be PII.',
    ],
    [
      /\b(?:\d[ -]?){13,16}\b/,
      'Possible Credit Card Number',
      'Response body may contain a credit card number pattern.',
    ],
    [
      /-----BEGIN (RSA |EC |OPENSSH )?PRIVATE KEY-----/,
      'Private Key in Response',
      'Response body contains a private key.',
    ],
    [
      /eyJ[A-Za-z0-9_-]{10,}\.[A-Za-z0-9_-]{10,}\.[A-Za-z0-9_-]{10,}/,
      'JWT Token in Response',
      'Response body contains a JWT token which may expose sensitive claims.',
    ],
  ];
  ...
  if (authHeader.toLowerCase().startsWith('basic '))
    issues.push({
      ...
      severity: 'medium',
      description:
        'Basic Auth credentials are base64-encoded (not encrypted). Prefer token-based auth.',
    });

  if (
    !authHeader &&
    !getHeader(reqH, 'cookie') &&
    !getHeader(reqH, 'x-api-key') &&
    req.method !== 'GET' &&
    req.method !== 'HEAD'
  )
    issues.push({
      id: 'no-auth',
      title: 'No Authentication Header',
      severity: 'info',
  ...
  if (acao === '*')
    issues.push({
      ...
      severity: 'medium',
      description: 'Access-Control-Allow-Origin: * allows any origin to read the response.',
      evidence: acao,
    });
  ...
  if (acao === '*' && acac.toLowerCase() === 'true')
    issues.push({
      ...
      severity: 'high',
      description:
        'Wildcard CORS with credentials=true is a critical misconfiguration allowing cross-origin credential theft.',
    });
  ...
  if (acao && acao !== '*' && getHeader(reqH, 'origin') && acao === getHeader(reqH, 'origin'))
    issues.push({
      ...
      severity: 'high',
      description:
        'Server reflects the request Origin back verbatim, allowing any origin to bypass CORS.',
      evidence: `Origin: ${getHeader(reqH, 'origin')} → ACAO: ${acao}`,
    });
  ...
  if (server && /[\d.]{3,}/.test(server))
    issues.push({
      ...
      severity: 'low',
      description: 'Server header reveals version info aiding fingerprinting.',
  ...
  const aspVersion =
    getHeader(res, 'x-aspnet-version') || getHeader(res, 'x-aspnetmvc-version') || '';
  if (aspVersion)
    issues.push({
      id: 'aspnet-version',
      title: 'ASP.NET Version Disclosed',
      severity: 'low',
      description: 'ASP.NET version header exposes framework version.',
      evidence: aspVersion,
    });
  ...
  if (/at\s+\w+[\w.]+\s*\(.*:\d+:\d+\)/m.test(resBody) || /stack\s*trace/i.test(resBody))
    issues.push({
      ...
      severity: 'high',
      description:
        'Response body contains a stack trace, exposing internal file paths and code structure.',
    });

  if (
    /SQL\s+(syntax|error|exception)/i.test(resBody) ||
    /ORA-\d{5}/.test(resBody) ||
    /mysql_fetch/i.test(resBody)
  )
    issues.push({
      id: 'sql-error',
      ...
      severity: 'high',
      description:
        'Response contains SQL error messages, indicating potential SQL injection vulnerability.',
    });
  ...
  if (/<script[\s>]/i.test(allInput) || /javascript:/i.test(allInput))
    issues.push({
      id: 'xss-in-request',
      title: 'Potential XSS Payload in Request',
      severity: 'high',
      description:
        'Request contains script tags or javascript: URI which may indicate XSS attempt.',
    });

  if (
    /(\bUNION\b.*\bSELECT\b|\bOR\b\s+['"]?\d+['"]?\s*=\s*['"]?\d+['"]?|--\s*$|;\s*DROP\s+TABLE)/i.test(
      allInput,
    )
  )
    issues.push({
  ...
  if (/\.\.[/\\]/.test(allInput))
    issues.push({
      id: 'path-traversal',
      title: 'Path Traversal Pattern in Request',
      severity: 'high',
      description:
        'Request contains ../ or ..\\ sequences indicating possible path traversal attempt.',
    });
  ...
  if (
    ...
    issues.push({
      id: 'sensitive-cacheable',
      title: 'Sensitive Response May Be Cached',
      severity: 'medium',
      description:
        'Authenticated/API response lacks Cache-Control: no-store or private. May be cached by proxies.',
  ...
  if (/http:\/\/(?!localhost|127\.0\.0\.1)/i.test(resBody))
    issues.push({
      id: 'mixed-content',
      title: 'Mixed Content in Response',
      severity: 'medium',
      description:
        'Response body references HTTP (non-HTTPS) URLs, causing mixed content warnings.',
    });
  ...
  if (!getHeader(res, 'cross-origin-opener-policy'))
    issues.push({
      ...
      severity: 'medium',
      description:
        'No COOP header. Page shares browsing context with cross-origin popups (Spectre risk).',
    });
  ...
  if (!getHeader(res, 'cross-origin-embedder-policy'))
    issues.push({
      ...
      severity: 'medium',
      description:
        'No COEP header. Required for SharedArrayBuffer and high-resolution timers isolation.',
    });
  ...
  if (getHeader(res, 'public-key-pins') || getHeader(res, 'public-key-pins-report-only'))
    issues.push({
      id: 'hpkp-deprecated',
      title: 'HPKP Header Detected (Deprecated)',
      severity: 'info',
      description:
        'HTTP Public Key Pinning is deprecated and removed from browsers. Remove Public-Key-Pins header.',
  ...
  if (req.method === 'TRACE')
    issues.push({
      id: 'trace-method',
      title: 'HTTP TRACE Method Used',
      severity: 'low',
      description:
        'TRACE method is enabled. Can be exploited for Cross-Site Tracing (XST) attacks.',
    });

  const acam = getHeader(res, 'access-control-allow-methods') || '';
  if (/\b(TRACE|TRACK)\b/i.test(acam))
    issues.push({
      id: 'trace-in-cors-methods',
      title: 'TRACE Allowed in CORS Methods',
      severity: 'low',
      description:
        'Access-Control-Allow-Methods includes TRACE/TRACK, enabling Cross-Site Tracing.',
      evidence: acam,
    });

  if (/\b(PUT|DELETE|PATCH)\b/i.test(acam))
    issues.push({
      id: 'dangerous-methods-exposed',
      title: 'Dangerous Methods in CORS',
      severity: 'low',
      description:
        'Access-Control-Allow-Methods exposes mutating methods (PUT/DELETE/PATCH) to cross-origin requests.',
      evidence: acam,
    });
