...
import {
  X, Search, Loader2, Monitor, RefreshCw, Smartphone, Wifi,
  Zap, Cloud, Usb, AlertCircle, Check, Terminal, Globe,
} from 'lucide-react';
...
interface AddTargetDrawerProps {
  ...
  onAdd: (app: {
    name: string; url?: string; executablePath?: string;
    mode: AppMode; platform: AppPlatform; icon?: string;
    emulatorSerial?: string; packageName?: string;
  }) => void;
...
export const AddTargetDrawer: React.FC<AddTargetDrawerProps> = ({
  isOpen, onClose, platform, onAdd, existingApps = [], editApp, onEdit,
}) => {
  // Web / CLI form
  ...
  // Android
  const [deviceList, setDeviceList] = useState<{ name: string; type: 'vm' | 'physical' | 'running-vm'; serial?: string }[]>([]);
  ...
  const [connectStatus, setConnectStatus] = useState<{ type: 'success' | 'error' | null; message: string }>({ type: null, message: '' });
  const [enablingWireless, setEnablingWireless] = useState<Record<string, boolean>>({});
  const [wirelessStatus, setWirelessStatus] = useState<Record<string, { ip?: string; message?: string }>>({});
  ...
  useEffect(() => {
    ...
    if (editApp) {
      ...
      setSelectedPcApp(null); setSelectedDevice(null);
      setPcSearch(''); setAndroidSearch('');
      setAndroidView('list');
    }
  ...
  const loadPcApps = async () => {
    setPcLoading(true);
    try { setDiscoveredApps(await window.api.invoke('apps:scan-pc')); }
    catch (e) { console.error(e); } finally { setPcLoading(false); }
  };
  ...
  const loadAndroidDevices = async () => {
    ...
    try {
      ...
      connected.forEach((dev: any) => list.push({ name: dev.name || dev.serial, type: dev.type === 'physical' ? 'physical' : 'running-vm', serial: dev.serial }));
      vms.forEach((vm: string) => { if (!connected.some((d: any) => d.name === vm || d.id === vm)) list.push({ name: vm, type: 'vm', serial: vm }); });
      setDeviceList(list);
    } catch (e) { console.error(e); } finally { setAndroidLoading(false); }
  };
  ...
  const handleEnableWireless = async (device: { name: string; serial?: string }) => {
    ...
    try {
      ...
      setWirelessStatus(p => ({ ...p, [serial]: result.success ? { ip: result.ip, message: result.message } : { message: result.error || 'Failed' } }));
      if (result.success) await loadAndroidDevices();
    } catch (e: any) { setWirelessStatus(p => ({ ...p, [serial]: { message: e.message } })); }
    finally { setEnablingWireless(p => ({ ...p, [serial]: false })); }
  };

  const handleWirelessConnect = async () => {
    if (!wirelessIp) { setConnectStatus({ type: 'error', message: 'Enter an IP address' }); return; }
    setIsConnecting(true); setConnectStatus({ type: null, message: '' });
    try {
      ...
      if (result.success) {
        ...
        setTimeout(() => { loadAndroidDevices(); setAndroidView('list'); }, 1500);
      } else setConnectStatus({ type: 'error', message: result.error || 'Failed' });
    } catch (e: any) { setConnectStatus({ type: 'error', message: e.message }); }
  ...
  const filteredPcApps = useMemo(() => {
    ...
    return discoveredApps.filter(a => a.name.toLowerCase().includes(l) || a.description?.toLowerCase().includes(l));
  }, [discoveredApps, pcSearch]);

  const filteredDevices = useMemo(() => {
    const existingSerials = new Set(existingApps.map(a => a.emulatorSerial).filter(Boolean));
    return deviceList.filter(d => d.serial && !existingSerials.has(d.serial) &&
  ...
  const handleSubmit = () => {
    if (isEdit && editApp && onEdit) {
      onEdit(editApp.id, { name, url: platform === 'web' ? url : undefined, executablePath: platform === 'cli' ? command : undefined });
      onClose(); return;
    }
    if (platform === 'web') {
      if (!name || !url) return;
      ...
      onAdd({ name: selectedPcApp.name, executablePath: selectedPcApp.exec, platform: 'pc', mode: 'native', icon: selectedPcApp.icon });
    } else if (platform === 'android') {
      if (!selectedDevice) return;
      const device = deviceList.find(d => (d.serial || d.name) === selectedDevice);
      if (!device) return;
      onAdd({ name: device.name, platform: 'android', mode: 'native', emulatorSerial: device.serial || device.name });
    }
    onClose();
  };

  const canSubmit = platform === 'web' ? !!(name && url)
    : platform === 'cli' ? !!(name && command)
    : platform === 'pc' ? !!selectedPcApp
    : !!selectedDevice;
    ...
    <>
      ...
      <div className="absolute bottom-0 left-0 right-0 z-50 bg-dialog-background border-t border-divider rounded-t-2xl shadow-2xl flex flex-col animate-in slide-in-from-bottom duration-300"
        ...
        <div className="px-4 pt-4 pb-3 border-b border-divider flex items-center gap-3 shrink-0">
          <div className={cn('flex items-center justify-center w-9 h-10 rounded-lg border shrink-0',
            platform === 'web'     ? 'bg-sky-500/15 border-sky-500/25' :
            platform === 'pc'      ? 'bg-violet-500/15 border-violet-500/25' :
            platform === 'android' ? 'bg-emerald-500/15 border-emerald-500/25' :
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-text-secondary mt-0.5">
              {isEdit ? 'Update target details' : `Configure a new ${platformMeta.label.toLowerCase()} target`}
            </p>
          </div>
          <button onClick={onClose} className="p-1.5 rounded-lg bg-secondary text-text-secondary hover:text-red-400 hover:bg-red-500/10 transition-all shrink-0">
            <X className="w-4 h-4" />
          </button>
        </div>
                ...
                {pcLoading && !discoveredApps.length ? (
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {filteredPcApps.map(app => {
                      const isSelected = selectedPcApp?.exec === app.exec && selectedPcApp?.name === app.name;
                      return (
                        <button key={`${app.name}-${app.exec}`} onClick={() => setSelectedPcApp(app)}
                          className={cn('flex items-center gap-3 p-3 rounded-xl border text-left transition-all',
                            isSelected ? 'bg-primary/10 border-primary/40' : 'bg-table-headerBg border-divider/40 hover:bg-sidebar-itemHover/60')}>
                          <div className="w-9 h-9 rounded-lg bg-secondary flex items-center justify-center shrink-0 overflow-hidden">
                            {app.icon ? <img src={`media://${app.icon}`} alt={app.name} className="w-8 h-8 object-contain" onError={e => { e.currentTarget.style.display = 'none'; }} />
                              : <Monitor className="w-4 h-4 text-text-secondary" />}
                          </div>
                          <div className="flex-1 min-w-0">
                            <div className="text-xs font-semibold text-text-primary truncate">{app.name}</div>
                            <div className="text-[10px] text-text-secondary truncate">{app.description || 'System App'}</div>
                          </div>
                        </button>
                      );
                    })}
              ...
              {androidView === 'list' ? (
                <>
                  <div className="p-3 border-b border-divider flex gap-2 shrink-0">
                    ...
                    <button onClick={loadAndroidDevices} className="p-2 bg-secondary hover:bg-sidebar-itemHover border border-divider rounded-lg">
                      <RefreshCw className={cn('w-4 h-4 text-text-secondary', androidLoading && 'animate-spin')} />
                    </button>
                  </div>
                  <div className="flex-1 overflow-y-auto p-3">
                    {androidLoading && !deviceList.length ? (
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        {filteredDevices.map(device => {
                          ...
                          const showWifi = device.type === 'physical' && !isWireless && !deviceList.some(d => d.name === device.name && d.serial?.includes(':5555'));
                          return (
                            <button key={serial} onClick={() => setSelectedDevice(serial)}
                              className={cn('relative flex flex-col p-3 rounded-xl border text-left transition-all',
                                isSelected ? 'bg-primary/10 border-primary/40' : 'bg-table-headerBg border-divider/40 hover:bg-sidebar-itemHover/60')}>
                              <div className="flex items-start justify-between mb-2">
                                <div className={cn('w-8 h-8 rounded-lg flex items-center justify-center',
                                  device.type === 'physical' ? 'bg-emerald-500/10 text-emerald-400' : device.type === 'running-vm' ? 'bg-blue-500/10 text-blue-400' : 'bg-secondary text-text-secondary')}>
                                  {device.type === 'physical' ? <Smartphone className="w-4 h-4" /> : device.type === 'running-vm' ? <Zap className="w-4 h-4" /> : <Monitor className="w-4 h-4" />}
                                </div>
                                {showWifi && (
                                  <button onClick={e => { e.stopPropagation(); handleEnableWireless(device); }} disabled={isEnabling}
                                    className="p-1.5 rounded-lg bg-secondary hover:bg-blue-500/20 hover:text-blue-400 text-text-secondary transition-all">
                                    {isEnabling ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Wifi className="w-3.5 h-3.5" />}
                                  </button>
                                )}
                              </div>
                              <div className="text-xs font-semibold text-text-primary truncate">{device.name}</div>
                              <div className="text-[10px] text-text-secondary mt-0.5">
                                {isWireless ? 'Wireless' : device.type === 'physical' ? 'USB' : device.type === 'running-vm' ? 'Running VM' : 'Stopped VM'}
                              </div>
                              {status?.message && !status?.ip && (
                                <div className="mt-1.5 px-2 py-1 bg-yellow-500/10 border border-yellow-500/20 rounded text-[9px] text-yellow-400">{status.message}</div>
                              )}
                            </button>
                          );
                        })}
                ...
                <div className="flex-1 p-5 space-y-4">
                  <button onClick={() => setAndroidView('list')} className="flex items-center gap-1.5 text-xs text-text-secondary hover:text-text-primary">
                    <X className="w-3.5 h-3.5" />Back to device list
                  </button>
                  <div>
                  ...
                  {connectStatus.type && (
                    <div className={cn('rounded-lg p-3 flex items-start gap-2 text-sm',
                      connectStatus.type === 'success' ? 'bg-green-500/10 border border-green-500/20 text-green-400' : 'bg-red-500/10 border border-red-500/20 text-red-400')}>
                      <AlertCircle className="w-4 h-4 mt-0.5 shrink-0" />{connectStatus.message}
                    </div>
                  )}
                  <button onClick={handleWirelessConnect} disabled={isConnecting || !wirelessIp}
                    ...
                    {isConnecting ? <><Loader2 className="w-4 h-4 animate-spin" />Connecting...</> : <><Check className="w-4 h-4" />Connect</>}
                  </button>
                </div>
              )}
            </div>
        ...
        <div className="px-5 py-4 border-t border-divider flex justify-end gap-3 shrink-0">
          <button onClick={onClose} className="px-4 py-2 rounded-lg text-sm font-medium text-text-secondary hover:text-text-primary hover:bg-sidebar-itemHover transition-colors">
            Cancel
          </button>
