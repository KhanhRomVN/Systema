...
import { Send, Save, X, Plus, List, Braces, FileText, Shield } from 'lucide-react';
import { cn } from '../../../../../shared/lib/utils';
import { CodeBlock, CodeBlockRef } from '../../../../../core/components/common/CodeBlock';
import { KeyValueTable } from './RequestComposer/KeyValueTable';
...
export function ComposerPanel({ appId, initialRequest, onClose }: ComposerPanelProps) {
  ...
  const [responseHeight, setResponseHeight] = useState(35); // percentage
  ...
  const handleMouseDown = (e: React.MouseEvent) => {
    ...
    dragStartHeight.current = responseHeight;
  ...
  useEffect(() => {
    const handleMouseMove = (e: MouseEvent) => {
      ...
      const deltaPercent = (delta / panelRect.height) * 100;
      let newHeight = dragStartHeight.current + deltaPercent;
      newHeight = Math.min(65, Math.max(25, newHeight));
  ...
  return (
    <div className="h-full flex flex-col bg-table-bodyBg">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-text-secondary mt-0.5">Temporary composer (not saved)</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            {onClose && (
      ...
      <div className="px-4 py-2 border-b border-divider/50 bg-table-headerBg/30 flex items-center justify-between shrink-0">
        <p className="text-xs text-text-secondary italic">Temporary comparison (not saved)</p>
        <button
          onClick={handleSaveToCollection}
          className="p-1.5 rounded-lg bg-secondary text-text-secondary hover:text-primary hover:bg-primary/10 transition-all"
      ...
      <div className="border-b border-divider shrink-0">
        <div className="flex items-stretch">
          {/* Method Dropdown - custom trigger with fit-width, no border radius */}
          <div className="relative" ref={methodDropdownRef}>
            <button
              onClick={() => setShowMethodDropdown(!showMethodDropdown)}
              className={cn(
                'h-10 px-3 border border-input-border-default border-r-0 text-sm font-bold outline-none cursor-pointer flex items-center gap-1 rounded-none',
          ...
          {/* URL Input - no border radius */}
          <input
            type="text"
            value={url}
            onChange={(e) => setUrl(e.target.value)}
            placeholder="https://api.example.com/endpoint"
            className="flex-1 h-10 px-3 bg-table-headerBg border-y border-input-border-default text-sm text-text-primary outline-none rounded-none"
            onKeyDown={(e) => e.key === 'Enter' && handleSend()}
          />

          {/* Send Button - no border radius */}
          <button
            onClick={handleSend}
            disabled={isLoading || !url}
            className={cn(
              'h-10 w-10 border border-input-border-default border-l-0 transition-all flex items-center justify-center rounded-none',
              isLoading || !url
          ...
          {tabs.map((tab) => {
            ...
            return (
              <button
                ...
                className={cn(
                  'py-2.5 text-xs font-medium transition-all text-center flex items-center justify-center gap-2',
                  isActive
      ...
      <div
        ref={containerRef}
        className="border-t border-divider shrink-0 flex flex-col"
        style={{ height: `${responseHeight}%`, minHeight: '25%', maxHeight: '65%' }}
      >
        {/* Resize handle */}
        <div
          className="h-1 cursor-ns-resize hover:bg-primary/50 transition-colors shrink-0"
          onMouseDown={handleMouseDown}
        />
        <div className="px-2 py-2 bg-table-headerBg border-b border-divider/50 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-2">
            {/* Response Tabs */}
            <button
