...
export function NetworkDetails({ request }: NetworkDetailsProps) {
  ...
  if (!analysis?.network) {
    return <div className="text-text-secondary italic text-xs">No network analysis available</div>;
  }
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
        ...
        <div className="p-2.5 bg-secondary/20 rounded-lg border border-divider/50">
          ...
          <div className="space-y-1.5 text-xs">
            <div className="flex justify-between border-b border-divider/30 pb-0.5">
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-1">
                {analysis.network.dns.resolvedIPs.map((ip, i) => (
                  <span
                    key={i}
                    className="bg-dialog-background border border-divider/50 px-1.5 py-0.5 rounded font-mono text-[10px]"
                  >
                    {ip.ip}
                  </span>
                ))}
        ...
        <div className="p-2.5 bg-muted/20 rounded-lg border border-border/50 md:col-span-2">
          ...
          {analysis.network.geolocation ? (
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-xs">
              <div>
                <div className="text-text-secondary mb-0.5 text-[10px]">Country</div>
              ...
              <div>
                <div className="text-text-secondary mb-0.5 text-[10px]">City</div>
                <div className="font-medium">
                  {analysis.network.geolocation.city}, {analysis.network.geolocation.region}
                </div>
              </div>
              <div>
                <div className="text-text-secondary mb-0.5 text-[10px]">ISP</div>
                <div className="font-medium truncate" title={analysis.network.geolocation.isp}>
                  {analysis.network.geolocation.isp}
                </div>
              </div>
              <div>
                <div className="text-text-secondary mb-0.5 text-[10px]">Timezone</div>
                <div className="font-mono">{analysis.network.geolocation.timezone}</div>
              </div>
            </div>
          ) : (
            <div className="text-text-secondary italic text-xs">Geolocation data not available</div>
          )}
        </div>
      </div>
    </div>
