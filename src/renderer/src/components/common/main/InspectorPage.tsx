...
export default function InspectorPage() {
  ...
  const handleStopSession = async () => {
    ...
    setRequests([]); // Clear requests on stop
  };
  ...
  const handleSelectApp = async (
    ...
    try {
      ...
      if (launched) {
        setSelectedApp(appName); // appName here is ACTUALLY the ID
  ...
  return (
    <div className="h-screen w-screen bg-background">
      <InspectorLayout
        onOpenTargetSelector={() => setIsTargetSelectorOpen(true)}
        requests={requests}
