...
export class ZenChatViewProvider implements vscode.WebviewViewProvider {
  ...
  constructor(
    ...
    // WebSocket server will be initialized externally in activate()
  }

  /**
   * Set WebSocket manager (called from activate())
   */
  public setWSManager(wsManager: WSManager): void {
    this._wsManager = wsManager;
    this._wsPort = wsManager.getPort();
  ...
  private getCheckpointService(): CheckpointService {
    if (!this._checkpointService) {
      ...
      const storagePath = this._extensionContext?.storageUri?.fsPath;

      this._checkpointService = new CheckpointService(rootPath, storagePath);
    }
  ...
  public async stopWebSocketServer(): Promise<void> {
    if (this._wsManager) {
      await this._wsManager.stop();
      this._wsManager = undefined;
    }
  }

  /**
   * Show current WebSocket server info (singleton cannot be restarted per-window)
   */
  public async restartWebSocketServer(): Promise<void> {
    // Singleton server cannot be restarted from individual windows
    // Just show current port info
    const port = this._wsPort;

    vscode.window
      .showInformationMessage(
        `Zen WebSocket server is running on port ${port} (shared across all windows)`,
        "Copy Port"
      )
      .then((selection) => {
        if (selection === "Copy Port") {
          vscode.env.clipboard.writeText(port.toString());
  ...
  public resolveWebviewView(
    ...
    webviewView.webview.onDidReceiveMessage(async (message) => {
      if (message.command === "getWorkspacePort") {
      ...
      } else if (message.command === "toggleProjectStructureBlacklist") {
        if (this._projectStructureManager) {
          await this._projectStructureManager.toggleBlacklist(
            message.path,
            message.isBlacklisted
          );
          // Send updated blacklist back
          const blacklist = await this._projectStructureManager.getBlacklist();
          webviewView.webview.postMessage({
            command: "projectStructureBlacklistResponse",
            blacklist,
            ...
            if (searchIndex === -1) {
              ...
              if (fuzzyMatch) {
                ...
                if (fuzzyMatch.score > 1e-9) {
                  ...
                  // matches might not be reliable if high score, but log what we found
                  console.log("Fuzzy Search Failed (Accuracy < 100%):");
                  console.log("1. Search Block Raw:", searchText);
                  console.log("2. Best Match Found:", fuzzyMatch.originalText);
                  console.log(`3. Accuracy: ${accuracy.toFixed(2)}%`);
              ...
              if (exactIndex !== -1) {
                ...
                if (fuzzyMatch) {
                  if (fuzzyMatch.score > 1e-9) {
                    ...
                    console.log("Fuzzy Search Validation:");
                    console.log("1. Search Block Raw:", searchText);
                    console.log(
                      "2. Search Block Found:",
                      fuzzyMatch.originalText
                    );
                    console.log(`3. Accuracy: ${accuracy.toFixed(2)}%`);
                    console.log(
                      `4. Diff: -${removedLines} lines, +${addedLines} lines`
                    );
                  }
      ...
      } else if (message.command === "checkProjectSize") {
        try {
          const service = this.getCheckpointService();
          const { totalSize, tree } = await service.calculateProjectSize();
          webviewView.webview.postMessage({
            command: "projectSizeResult",
            requestId: message.requestId,
            size: totalSize,
            tree: tree,
          });
        } catch (error) {
          console.error("Failed to check project size:", error);
        }
      } else if (message.command === "createCheckpoint") {
        try {
          // 🆕 Check setting
          const enableCheckpoints = await this._storageManager?.get(
            "enableCheckpoints"
          );
          ...
          const service = this.getCheckpointService();
          // Message format: { command: 'createCheckpoint', type: 'full' | 'incremental', messageId?: string, parentId?: string }
          ...
          if (result) {
            webviewView.webview.postMessage({
              command: "createCheckpointResult",
              requestId: message.requestId,
              result,
            });
          }
          // If null (skipped), we do nothing or send a skipped message.
          // Doing nothing is fine, ChatPanel won't add a message.
        } catch (error) {
          webviewView.webview.postMessage({
            command: "createCheckpointResult",
            requestId: message.requestId,
      ...
      } else if (message.command === "restoreCheckpoint") {
        try {
          const service = this.getCheckpointService();
          const messageId = await service.restoreCheckpoint(
            message.checkpointId,
            message.restoreFilesMode || "changed_only"
          );

          // Only return messageId if restoreChat is true (or undefined/default)
          // If restoreChat is explicitly false, we send undefined so frontend doesn't truncate
          const finalMessageId =
            message.restoreChat !== false ? messageId : undefined;
        ...
        try {
          ...
          setTimeout(async () => {
            try {
              const updatedContent = await vscode.workspace.fs.readFile(
                fileUri
              );
              const updatedString =
        ...
        try {
          ...
          vscode.workspace.findFiles("**/*", "**/node_modules/**").then(
            async (files) => {
              try {
                ...
                files.forEach((file) => {
                  ...
                  if (folderPath && folderPath !== ".") {
                    ...
                    for (let i = 1; i <= parts.length; i++) {
                      const folder = parts.slice(0, i).join(path.sep);
...
<body>
 ...
 <script nonce="${nonce}" src="${scriptUri}">
  ...
  public setExtensionContext(context: vscode.ExtensionContext): void {
    ...
    this._storageManager = new GlobalStorageManager(context);
...
export async function activate(context: vscode.ExtensionContext) {
  const outputChannel = vscode.window.createOutputChannel("Zen Debug");

  outputChannel.appendLine(
    "---------------------------------------------------"
  );
  outputChannel.appendLine(
    `[${new Date().toISOString()}] Activating Zen Extension...`
  );
  outputChannel.appendLine(
    `Activation Event: ${JSON.stringify(
      context.extension.packageJSON.activationEvents
    )}`
  );
  outputChannel.appendLine(`Extension Path: ${context.extensionPath}`);

  // Initialize Managers
  const storageManager = new GlobalStorageManager(context);
  ...
  // Retrieve previous session state for this window
  let port: number;
  try {
    port = await PortManager.getAvailablePort();
  } catch (error) {
    vscode.window.showErrorMessage(
      `Failed to generate port: ${
        error instanceof Error ? error.message : String(error)
      }`
    );
    return;
  ...
  try {
    ...
    // Show notification with port number
    vscode.window
      .showInformationMessage(
        `Zen WebSocket server started on port ${port}`,
        "Copy Port"
      )
      .then((selection) => {
        if (selection === "Copy Port") {
  ...
  provider.setWSManager(wsManager);
