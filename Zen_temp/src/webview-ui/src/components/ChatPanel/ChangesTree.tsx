...
const ChangesTree: React.FC<ChangesTreeProps> = ({ messages }) => {
  ...
  const renderNode = (node: FileNode) => {
    const isExpanded = expandedFolders.has(node.path) || true; // Default expanded for now? Or implement proper toggle.
    // Let's default expand top level?
    // Actually, `expandedFolders` logic needs to start empty or full.
    // Let's make folders expanded by default by checking !has instead of has, or initializing full.
    // Simpler: Click toggles. Default collapsed? Default expanded is better for visibility.
  ...
  // Initialize folders as expanded effects
  // We'll use a simplistic "Auto expand all" for now on mount or changes?
  // Or just let user click.
  // Better: Default expanded.
    ...
    return (
      <div style={{ maxHeight: "300px", overflowY: "auto", padding: "4px 0" }}>
        {Object.values(tree)
          ...
          .map((node) => {
            // Force expand root level folders initially?
            // To do this cleanly, we need `useEffect` to populate `expandedFolders`.
            // But simpler: just treat them as open if not in "closed" set.
            // Let's swap logic: `collapsedFolders` set.

            // For now, let's keep it manual.
            return (
              <div key={node.path}>
                {/* Manually rendering root node slightly differently? No. */}
                {renderNode({
                  ...node,
                  // HACK: Pass a prop or just rely on state.
                  // Let's rely on state. User has to click to open folders.
                })}
              </div>
            );
          })}
