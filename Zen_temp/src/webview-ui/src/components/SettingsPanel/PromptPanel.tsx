...
const getPromptModules = (language: string): PromptModule[] => {
  const gitLanguage = language === "vietnamese" ? "vi" : "en";
  ...
  return [
    ...
    // Git
    {
      ...
      description: "Conventional commits format, types, and best practices",
      emoji: "📝",
      filePath: "git.ts",
      category: "rules",
      content: PromptModules.getGitPrompt(gitLanguage),
    },
    // System
    {
      id: "system_info",
...
const PromptPanel: React.FC<PromptPanelProps> = ({ onBack }) => {
  ...
  // Get modules based on selected language
  const PROMPT_MODULES = getPromptModules(selectedLanguage);
